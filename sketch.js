...
function preload(){
  font = loadFont('assets/BroadwayRegular-7Bpow.ttf')
}
...
function drawWords() {
  ...
  if(snake.wingame()){
    ...
    text('Congrats!!!', w/2, h/2)

  }else if(snake.endGame()) {
  textSize(4.5)
  text('Game Over!', w/2, h/2 - 2.5) 
  textSize(3)
  text('Final score:', w/2, h/2 + 1.25)
  text(snake.body.length - 1, w/2, h/2 + 4)

  }else{

  text('EPQ Project 2022', w/2, h/2-1);
  text('Snake AI', w/2, h/2)
  text('By Lucas Cunningham', w/2, h/2+1)
  text('Score: ', w/2, h/2+2)
  text(snake.body.length - 1, w/2, h/2+3)
  console.log('draw words')

  }

  
  //function that when called in funtion draw(), writes out the title text
